  ...
  var S = {
    ...
    _check_pos: function () {
      ...
      if (win_top > (S.dims.nav_offset - S.settings.fixedMargin)) {
        $nav.addClass('fixed');
        if (S.settings.onFixed) { S.settings.onFixed.call(this); }
      }
      else {
        $nav.removeClass('fixed');
        if (S.settings.onUnfixed) { S.settings.onUnfixed.call(this); }
      }
      ...
      if (S.settings.stickInParent) {
        var $wrapper = $nav.find('.' + S.settings.className + '__wrapper');
        if (!sections_active.length) {
          $wrapper.css({
            position: 'absolute',
            bottom: 0,
            top: 'auto'
